...
function HomePage() {
            ...
            Object.keys(dateInfo).forEach((date) => {
              dateInfo[date].sort((a, b) => {
                ...
                const priorityA = a.promoters.reduce((min, promoter) => {
                  if (promoter.priority !== null && (min === null || promoter.priority < min)) {
  ...
  const elementDivCard =
    ...
    dataEventCard.map((event, i) => {
      ...
      let capitalizedDay =
        formattedDate.charAt(0).toUpperCase() + formattedDate.slice(1, formattedDate.indexOf(','))
      ...
      return (
        <div key={i} className={`${styles.containerEventCard} ${additionalClass}`}>
          <Helmet>
          ...
          <h1 ref={(el) => (headersRef.current[i] = el)} className={styles.stickyHeader}>
            {finalFormattedDate}
          </h1>
  ...
  const onCloseUbicaion = () => {
    window.scroll(0, 0)
    let resetCheckedItems = Object.keys(checkedItemsCities).reduce((acc, key) => {
      acc[key] = false
      return acc
    }, {})
  ...
  const onClickCheckBoxListUbicacion = (item) => {
    ...
    if (citieName[0] === 'Ubicación') {
      ...
      if (arrayCitiesName.includes(item.city_name)) {
        arrayCitiesName = arrayCitiesName.filter((name) => name !== item.city_name)
      } else {
        arrayCitiesName.push(item.city_name)
      }
    }
            ...
            if (isDateInArray) {
              newArray.forEach((existingData) => {
                ...
                if (existingDate === newDate) {
                  existingData[newDate] = existingData[newDate].concat(newData[newDate])
                }
              })
            } else {
    ...
    return (
      <div className={styles.body}>
        ...
        <div className={styles.containerFixed} id="containerFixed">
          ...
          <div className={styles.containerButtons}>
            {openUbicacion || (citieName.length && citieName[0] !== 'Ubicación') ? (
