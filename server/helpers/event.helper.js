...
const {
  getImageFromCache,
} = require("../utils/cacheFunction/cacheFunction.js");
...
class EventHelper {
  ...
  async createEventByForm(data) {
    ...
    if (!name && event_djs.length > 0) {
      name = event_djs[0]?.name
        ? event_djs?.map((dj) => dj?.name).join(" | ")
        : null;
    }
  ...
  async updateEvent(id, data, event) {
    ...
    let typesIDs =
      event_type.length > 0
        ? event_type.map((type) => type.id).filter((id) => id)
        : null;
    let djsIDs =
      event_djs.length > 0
        ? event_djs.map((dj) => dj.id).filter((id) => id)
        : null;
    let promotersIDs =
      event_promoter.length > 0
    ...
    if (promotersIDs && promotersIDs[0]) {
      await facade2.updateRelationshipEvent(
        eventInstance,
        "setPromoters",
        promotersIDs
      );
