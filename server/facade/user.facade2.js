...
class UserFacade2 {
  ...
  async createUserInClerk(email, password, username) {
    //TODO boton > email > nueva pass >
    // update user desde la app, si quiere updatear la pass
    //manda la nueva pass la updateo en nuestra bd y le paso la nueva pass a clerk
    //agregar ruta send-email-new-pass-app que envia un email con la ruta a la pantalla del updatepass
    //y de ahi una ruta al update user.
    try {
  ...
  async updateUserInClerk(userId, data) {
    try {
      ...
      console.log('User password updated in Clerk:', user)
      return user
    } catch (error) {
      console.error('Error updating user password in Clerk:', error)
      return null
    }
  }
