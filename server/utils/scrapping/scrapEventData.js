...
const linkScrapping = async (link) => {
  ...
  try {
    if (SCRAPPING) {
      ...
      // console.log('1')
      // console.log('1_ browser:', browser)
    } else {
      browser = await puppeteer.launch({
    ...
    if (link.includes('ticketpass')) {
      ...
      if (sectionTittleFecha) {
        try {
          ...
          dateText = await sectionTittleFecha.$eval('h6', (h6) => h6.textContent.trim())
      ...
      if (sectionLocation) {
        try {
          location = await sectionLocation.evaluate((p) => p.textContent.trim())
        } catch (error) {
          location = ''
          console.log(error)
        }
        ...
        var imageSrc = await page.evaluate(() => {
          ...
          for (const img of imgElements) {
            ...
            if (src && src.includes('cloudfront.net')) {
              ...
              return src.startsWith('http') ? src : `https://www.ticketpass.com.ar${src}`
            }
          }
          return ''
        })
      ...
      try {
        ...
        try {
          jpgImgSrc = await page.evaluate(() => {
            ...
            return imgElement && imgElement.src.toLowerCase().endsWith('.jpg') ? imgElement.src : ''
          })
        } catch (error) {
          jpgImgSrc = ''
          console.log(error)
        ...
        const results1 = await page.evaluate(() => {
          ...
          elements.forEach((element) => {
            ...
            } else if (element.querySelector('svg[data-testid="AccessTimeIcon"]')) {
